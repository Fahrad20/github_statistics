...
describe("Test /api/", () => {
  ...
  it("should have proper cache", async () => {
    ...
    expect(res.setHeader.mock.calls).toEqual([
      ...
      [
        "Cache-Control",
        `max-age=${CONSTANTS.SIX_HOURS / 2}, s-maxage=${CONSTANTS.SIX_HOURS
  ...
  it("should set proper cache", async () => {
    ...
    expect(res.setHeader.mock.calls).toEqual([
      ...
      [
        "Cache-Control",
        `max-age=${cache_seconds / 2
        }, s-maxage=${cache_seconds}, stale-while-revalidate=${CONSTANTS.ONE_DAY
        }`,
      ],
    ]);
  });
  ...
  it("should set proper cache with clamped values", async () => {
    {
      ...
      expect(res.setHeader.mock.calls).toEqual([
        ...
        [
          "Cache-Control",
          `max-age=${CONSTANTS.ONE_DAY / 2}, s-maxage=${CONSTANTS.ONE_DAY
          }, stale-while-revalidate=${CONSTANTS.ONE_DAY}`,
        ],
      ]);
    }
